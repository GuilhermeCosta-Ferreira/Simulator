...
Gaussian via the injected NumPy Generator. Sampling is made deterministic in
tests by passing a seeded Generator.
"""
...
def test_sample_returns_float() -> None:
    ...
    assert isinstance(dist.sample(np.random.default_rng(0)), float)
...
def test_sample_is_reproducible_under_same_seed() -> None:
    ...
    first = dist.sample(np.random.default_rng(123))
    second = dist.sample(np.random.default_rng(123))
