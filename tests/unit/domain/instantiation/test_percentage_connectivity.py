...
def _build_matrix(n: int, percentage: float) -> np.ndarray:
    ...
    return factory.build_connectivity_matrix(nodes, blueprint).data
...
def test_build_targets_full_population_for_first_node() -> None:
    ...
    np.random.seed(0)
    result = rule.build(node_id=0, connection_dict=connection_dict)
...
def test_build_only_samples_forward_nodes() -> None:
    ...
    np.random.seed(0)
    result = rule.build(node_id=2, connection_dict=connection_dict)
...
def test_build_returns_empty_when_already_connected_enough() -> None:
    ...
    result = rule.build(node_id=3, connection_dict=connection_dict)
...
def test_build_returns_empty_when_no_forward_candidates() -> None:
    ...
    result = rule.build(node_id=4, connection_dict=connection_dict)
...
def test_build_is_reproducible_under_a_fixed_seed() -> None:
    ...
    np.random.seed(123)
    first = rule.build(node_id=0, connection_dict=connection_dict)
    np.random.seed(123)
    second = rule.build(node_id=0, connection_dict=connection_dict)
...
def test_matrix_stays_symmetric() -> None:
    ...
    np.random.seed(7)
    matrix = _build_matrix(n=40, percentage=0.5)
...
def test_mean_connectivity_stays_within_ten_percent_of_target(
    ...
    fractions = [
        _mean_connectivity_fraction(_build_matrix(n=n, percentage=percentage))
        for _ in range(runs)
    ]
